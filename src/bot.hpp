...
constexpr int32_t CardPower(Rank rank)
{
    constexpr int32_t card_power[] = {0, 1, 6, 4, 7, 2, 3, 5};
    return card_power[(int32_t)rank - (int32_t)Rank::SEVEN];
}
...
struct PlayAction
{
    ...
    PlayAction() = default;
    PlayAction(Action a,const Card& c)
    {
        action = a;
        played_card = c;
    }

    constexpr bool operator==(const PlayAction &other) const
    {
        if(action == Action::RevealTrump || action == Action::NoAction)
            return action == other.action;
        else
            return action == other.action && played_card == other.played_card;
    }
};
